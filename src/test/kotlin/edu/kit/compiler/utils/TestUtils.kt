...
object TestUtils {
    ...
    internal inline fun <T> withParser(input: String, f: Parser.() -> T): T {
    ...
    internal inline fun <reified T : AST> expectNode(
        input: String,
        expectedNode: Parsed<T>,
        runParser: Parser.() -> Parsed<T>
    ) {
        val res = withParser(input) { runParser() }
    ...
    internal inline fun <reified T : AST> expectNode(
        input: String,
        expectedNode: List<Parsed<T>>,
        runParser: Parser.() -> List<Parsed<T>>
    ) {
        val res = withParser(input) { runParser() }
        assertEquals(expectedNode.debug(), res.debug())
    }
