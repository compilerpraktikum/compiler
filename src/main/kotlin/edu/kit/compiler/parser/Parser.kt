...
private fun SourceRange.extendUntilFirstValid(vararg ranges: SourceRange?): SourceRange =
    this.extend(ranges.firstNotNullOfOrNull { it } ?: throw IllegalStateException("all ranges invalid"))
...
class Parser(sourceFile: SourceFile, tokens: Sequence<Token>) :
    ...
    /**
     * @param isParenthesized Only used for putting parentheses into the AST (only for literals).
        ...
        return when (val peekedToken = peek()) {
            ...
            is Token.Operator -> {
                if (peekedToken.type == Token.Operator.Type.LParen) {
                    next()
                    val innerExpr =
                        parseExpression(1, anc + anchorSetOf(Token.Operator(Token.Operator.Type.RParen)), true)
                    val rParen = expectOperator(Token.Operator.Type.RParen, anc) { "expected closing parenthesis." }
                    innerExpr.mapPosition { peekedToken.range.extend(rParen.range) }
                } else {
                    reportError(
                        peekedToken,
            ...
            is Token.Keyword -> {
                when (peekedToken.type) {
                    ...
                    Token.Keyword.Type.New -> {
                        next()
                        parseNewObjectArrayExpression(anc, isParenthesized).mapPosition { peekedToken.range.extend(it) }
                    }
                    else -> {
                        reportError(
                            peekedToken,
    ...
    private fun parseNewArrayExpression(anc: AnchorUnion, isParenthesized: Boolean): Parsed<AST.Expression> {
        ...
        val indexExpression = parseExpression(
            anc = anc + anchorSetOf(Token.Operator(Token.Operator.Type.RightBracket)),
    ...
    private fun parseArguments(anc: AnchorUnion): List<Parsed<AST.Expression>> {
        ...
        while (nextToken !in loopAnchor) {
            ...
            arguments += parseExpression(
                anc = anc + anchorSetOf(Token.Operator(Token.Operator.Type.RParen)),
                ...
                when (firstPeekedToken.type) {
                    Token.Operator.Type.Dot -> {
                        ...
                        if (maybeLParent is Token.Operator && maybeLParent.type == Token.Operator.Type.LParen) {
                            ...
                            parsePostfixOp(
                                AST.MethodInvocationExpression(target, parsedIdent, arguments).wrapConditionally(
                                    rightParen.isValid,
                                    target.range.extendUntilFirstValid(
                                        rightParen.getAsValid()?.range,
                                        arguments.lastOrNull()?.range,
                                        maybeLParent.range
                                    )
                                ),
                                anc
                    ...
                    Token.Operator.Type.LeftBracket -> {
                        ...
                        val accessExpression = AST.ArrayAccessExpression(target, index).wrapConditionally(
                            rBracket.isValid,
                            target.range.extendUntilFirstValid(rBracket.getAsValid()?.range, index.range)
                        )
    ...
    internal fun parseFieldMethodPrefix(anc: AnchorUnion): Parsed<AST.ClassMember> {
        val type = parseType(
            anc +
                anchorSetOf(
                    Token.Identifier.Placeholder,
        ...
        return when (val fieldMethodRestToken = peek(0)) {
            is Token.Operator -> {
                when (fieldMethodRestToken.type) {
                    Token.Operator.Type.Semicolon -> parseField(ident, type, fieldMethodRestToken.range)
    ...
    private fun parseField(
        ...
        return AST.Field(
            ident.map { it.name },
            type
        ).wrapValid(type.range.extend(semicolonRange))
    }
    ...
    private fun parseMethod(
        ...
        if (closingParenthesis.isValid) {
            return AST.Method(
                ...
                block,
                throwsException
            ).wrapValid(type.range.extend(block.range))
        } else {
            return AST.Method(
                ident.map { it.name },
                type,
                ...
                block,
                throwsException
            ).wrapErroneous(type.range.extend(block.range))
        }
    }
    ...
    internal fun parseBlock(anc: AnchorUnion): Parsed<AST.Block> {
        ...
        val resultBlock = AST.Block(
            blockStatements,
            Unit.wrapConditionally(openingBrace.isValid, openingBrace.range),
            Unit.wrapConditionally(closingBrace.isValid, closingBrace.range)
        )
    ...
    private fun parseReturnStatement(returnKeyword: Token, anc: AnchorUnion): Parsed<AST.ReturnStatement> {
        ...
        val endingSemicolonRange = if (maybeSemicolon is Token.Operator && maybeSemicolon.type == Token.Operator.Type.Semicolon) {
            next().range
        } else {
            returnValue = parseExpression(
                anc = anc + anchorSetOf(Token.Operator(Token.Operator.Type.Semicolon)),
                isParenthesized = false
            )
            val semicolon =
                expectOperator(Token.Operator.Type.Semicolon, anc) { "expected `;` after return expression" }
        ...
        return AST.ReturnStatement(returnValue)
            .wrapValid(returnKeyword.range.extend(endingSemicolonRange))
    }
    ...
    private fun parseEmptyStatement(tokenSemicolon: Token): Parsed<AST.Statement> {
        ...
        return AST.Block(
            listOf(),
            Unit.wrapValid(tokenSemicolon.range),
            Unit.wrapValid(tokenSemicolon.range)
        ).wrapValid(tokenSemicolon.range)
    }
