...
private fun SourceRange.extendUntilFirstValid(vararg ranges: SourceRange?): SourceRange = ranges.firstNotNullOfOrNull { it } ?: throw IllegalStateException("all ranges invalid")
                        ...
                        if (maybeLParent is Token.Operator && maybeLParent.type == Token.Operator.Type.LParen) {
                            ...
                            parsePostfixOp(
                                AST.MethodInvocationExpression(target, parsedIdent, arguments).wrapConditionally(
                                    rightParen.isValid,
                                    target.range.extendUntilFirstValid(rightParen.getAsValid()?.range, arguments.lastOrNull()?.range, maybeLParent.range)
                                ),
                                anc
                            )
                        } else {
