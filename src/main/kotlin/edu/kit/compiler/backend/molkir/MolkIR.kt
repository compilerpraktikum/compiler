...
private constructor(
    ...
    constructor(const: Int, base: InputOutputTarget, index: InputOutputTarget? = null, scale: Int? = null) : this(const as Int?, base, index, scale)
    constructor(base: InputOutputTarget, index: InputOutputTarget? = null, scale: Int? = null) : this(null, base, index, scale)
...
abstract class Instruction : MolkIR {
    ...
    class BinaryOperationWithResult(val name: String, val left: InputOutputTarget, val right: InputOutputTarget, val result: Target) : Instruction() {
        override fun toMolki(): String = "$name [ ${left.toMolki()} | ${right.toMolki()} ] -> ${result.toMolki()}"
    }

    class BinaryOperationWithTwoPartResult(val name: String, val left: InputOutputTarget, val right: InputOutputTarget, val resultLeft: Target, val resultRight: Target) : Instruction() {
        override fun toMolki(): String = "$name [ ${left.toMolki()} | ${right.toMolki()} ] -> [ ${resultLeft.toMolki()} | ${resultRight.toMolki()} ]"
    }
}
